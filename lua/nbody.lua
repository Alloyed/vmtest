...
local all_bodies = {sun,jupiter,saturn,uranus,neptune}
...
offsetMomentum(all_bodies, nbody)
energy(all_bodies, nbody)
--io.write( string.format("%0.9f",energy(all_bodies, nbody)), "\n")
for _=1,N do advance(all_bodies, nbody, 0.01) end
energy(all_bodies, nbody)
--io.write( string.format("%0.9f",energy(all_bodies, nbody)), "\n")
